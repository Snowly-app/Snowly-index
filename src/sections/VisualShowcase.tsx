...
export function VisualShowcase() {
  ...
  const [hoverSpot, setHoverSpot] = useState({ x: 0, y: 0, opacity: 0 });
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setTilt(clampedProgress * 6); // Max tilt 6 degrees for subtitle elegance
    };
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setHoverSpot({ x, y, opacity: 1 });
  };

  const handleMouseLeave = () => {
    setHoverSpot(prev => ({ ...prev, opacity: 0 }));
  };

  return (
    <section ref={containerRef} id="showcase" className="py-24 relative overflow-hidden bg-zinc-950 perspective-[2000px]">
      <div className="container mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-16 md:mb-24">
          ...
          <p className="text-xl text-white/50 font-light">
            Detailed run analysis with real-time speed curves, high-frequency GPS logging, and automatic run detection.
            Snowly turns your ride into hard numbers.
          </p>
        </div>

        <div className="relative mx-auto max-w-5xl flex justify-center items-center gap-8 md:gap-16 perspective-[2000px]">
          {/* Parallax Background Elements */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-primary/10 rounded-full blur-[160px] -z-10 mix-blend-screen pointer-events-none" />
          
          {/* Main Device Mockup (iPhone) */}
          <div 
            ref={mockupRef}
            onMouseMove={handleMouseMove}
            onMouseLeave={handleMouseLeave}
            className="group relative z-10 w-[340px] md:w-[380px] h-[720px] md:h-[800px] rounded-[3.5rem] md:rounded-[4rem] border-[14px] border-zinc-900 bg-black shadow-[0_0_100px_rgba(0,0,0,0.8),inset_0_0_0_1px_rgba(255,255,255,0.1)] overflow-hidden transition-all duration-300 ease-out"
            style={{ 
              transform: `rotateX(${tilt}deg) scale(1.02)`,
          ...
          >
            ...
            <div 
              ...
              style={{
                opacity: hoverSpot.opacity,
                background: `radial-gradient(400px circle at ${hoverSpot.x}px ${hoverSpot.y}px, rgba(255,255,255,0.15), transparent 40%)`
              }}
            />
            ...
            <div className="absolute top-0 inset-x-0 h-9 flex justify-center z-40 pointer-events-none">
                <div className="w-[120px] h-full bg-black rounded-b-3xl relative">
                    <div className="absolute top-2 right-4 w-2.5 h-2.5 rounded-full bg-zinc-800 border border-zinc-700/50 shadow-inner flex items-center justify-center">
                        <div className="w-1 h-1 rounded-full bg-blue-900/50 blur-[0.5px]"></div>
                    </div>
                </div>
            </div>
          ...
          <div className="hidden lg:block relative z-20 w-[180px] h-[220px] rounded-[2.5rem] border-[10px] border-zinc-800 bg-black shadow-[0_20px_50px_rgba(0,0,0,0.5),inset_0_0_0_1px_rgba(255,255,255,0.2)] transform -rotate-[10deg] hover:rotate-0 hover:scale-110 transition-all duration-700 ease-out translate-y-16 -ml-16 animate-float">
             {/* Crown */}
